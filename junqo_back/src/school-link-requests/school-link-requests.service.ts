...
export class SchoolLinkRequestsService {
  ...
  async create(
    ...
    if (currentUser.type !== UserType.STUDENT) {
      throw new ForbiddenException(
        'Only students can request to link with a school',
      );
    }

    // Check if student already has a pending request
    const existingPending = await this.repository.findPendingByStudent(
      currentUser.id,
    );
    if (existingPending) {
    ...
    const studentProfile = await this.studentProfilesRepository.findOneById(
      currentUser.id,
    );
    ...
    try {
      ...
      throw new InternalServerErrorException(
        `Failed to create school link request: ${_error.message}`,
      );
    }
  }
  ...
  async getMyRequests(
    ...
    if (currentUser.type !== UserType.STUDENT) {
      throw new ForbiddenException(
        'Only students can view their link requests',
      );
    }
  ...
  async getPendingRequests(
    ...
    if (currentUser.type !== UserType.SCHOOL) {
      throw new ForbiddenException(
        'Only schools can view pending link requests',
      );
    }
  ...
  async accept(
    ...
    if (request.schoolId !== currentUser.id) {
      throw new ForbiddenException(
        'You can only accept requests sent to your school',
      );
    }
  ...
  async reject(
    ...
    if (request.schoolId !== currentUser.id) {
      throw new ForbiddenException(
        'You can only reject requests sent to your school',
      );
    }
  ...
  async cancel(currentUser: AuthUserDTO, requestId: string): Promise<boolean> {
    if (currentUser.type !== UserType.STUDENT) {
      throw new ForbiddenException(
        'Only students can cancel their link requests',
      );
    }
  ...
  async findOneById(
    ...
    if (
      ...
      throw new ForbiddenException(
        'You can only view requests sent to your school',
      );
    }
